...
class ChatActivity : AppCompatActivity() {
            ...
            if (value != null) {
                ...
                for (documentChange in value.documentChanges) {
                    if (documentChange.type == DocumentChange.Type.ADDED)
                    {
                        ...
                        chatMessage.recieverId =  documentChange.document.getString(Constants.KEY_RECIEVER_ID).toString()
                        chatMessage.message =  documentChange.document.getString(Constants.KEY_MESSAGE).toString()
                        chatMessage.dateTime =  getDateTime(documentChange.document.getDate(Constants.KEY_TIMESTAMP) as Date)
                        chatMessage.dateObject =  documentChange.document.getDate(Constants.KEY_TIMESTAMP) as Date
    ...
    private fun sendMessage()
    {
        ...
        map[Constants.KEY_RECIEVER_ID] = receiverUser.id
        map[Constants.KEY_MESSAGE] = binding.inputMessage.text.toString()
        map[Constants.KEY_TIMESTAMP] = time.toString() // Possible no need to parse?
